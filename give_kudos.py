...
class KudosGiver:
    ...
    def send_telegram_message(self, message, token=self.tg_token, chat_id=self.tg_chat_id):
    ...
    def locate_kudos_buttons_and_maybe_give_kudos(self, web_feed_entry_locator) -> int:
            ...
            if p_count > 1:
                for j in range(p_count):
                    ...
                    if not self.is_participant_me(participant):
                        send_telegram_message(participant)
        ...
        send_telegram_message(f"Kudos given: {given_count}")
        return given_count
